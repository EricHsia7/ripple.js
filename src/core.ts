...
function generateID(): string {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    id += idchars.substring(idrandomNumber, idrandomNumber + 1);
  }
  return id;
}

function rippleEventHandler(event) {
  let ripple_id = generateID();
...
function __addToSingleElement(element: HTMLElement, color: string, duration: number, callback?: Function): void {
  ...
  const trackingID = `ripple_${generateID()}`;
  element.setAttribute('ripple-id', trackingID);
  if (!ripple.trackingList.hasOwnProperty(trackingID)) {
    ripple.trackingList[trackingID] = {
      trackingID,
      element,
      color,
      duration,
    ...
    element.addEventListener(eventlistener, rippleEventHandler);
  }
}
...
function __removeFromSingleElement(element: HTMLElement): void {
  const trackingID = String(element.getAttribute('ripple-id'));
  const regex = /^ripple_[0-9a-z]{16,16}$/gm;
